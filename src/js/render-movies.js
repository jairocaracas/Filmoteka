...
import { setWatchedMovies, setQueuedMovies, loadMoviesFromLocalStorage } from "./fetch-movie.js";
...
async function renderMovieCards(fetchFunction, div) {
  ...
  div.innerHTML += newRender.join(' ');
		...
		movieCards.forEach(movieCard => {
			...
			let cardQueueButton = cardButtons.querySelector('.movie-card__buttons--queue');
			let cardWatchedButton = cardButtons.querySelector('.movie-card__buttons--watched');
			...
			cardQueueButton.addEventListener('click', () => {
				...
				if (setQueuedMovies(cardQueueButton.dataset.id, cardQueueButton.dataset.title)) {
					cardQueueButton.classList.add('activebutton-queue');
					cardQueueButton.classList.remove('inactivebutton-queue');
				}
				else {
			...
			cardWatchedButton.addEventListener('click', () => {
				loadMoviesFromLocalStorage();
				if (setWatchedMovies(cardWatchedButton.dataset.id, cardWatchedButton.dataset.title)) {
					cardWatchedButton.classList.add('activebutton-watched');
					cardWatchedButton.classList.remove('inactivebutton-watched');
				}
				else {
